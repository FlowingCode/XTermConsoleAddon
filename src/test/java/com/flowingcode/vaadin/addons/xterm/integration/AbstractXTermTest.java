...
public abstract class AbstractXTermTest extends AbstractViewTest {
	...
	final int getColumnWidth(TestBenchElement term) {
		return ((Long)getCommandExecutor().executeScript("return arguments[0].terminal.cols", term)).intValue();
	}
	
	final void assertFeatureHasBeenLoaded(TestBenchElement term, Class<? extends XTermFeature> featureClass, String featureName) {
		Object property = getCommandExecutor().executeScript("return arguments[0].features[arguments[1]]", term, featureName);		
	...
	final String lineAtOffset(TestBenchElement terminal, int offset) {
		String command = "buffer=arguments[0].terminal._core._inputHandler._bufferService.buffer; return buffer.lines.get(buffer.ybase+buffer.y+(%s)).translateToString()";
		command = String.format(command, offset);
		return ((String) getCommandExecutor().executeScript(command, terminal)).replaceFirst("\\s+$", "");
	}
	...
	final Position cursorPosition(TestBenchElement terminal) {
		int[] pos = intArray(getCommandExecutor().executeScript("buffer=arguments[0].terminal.buffer; return [buffer.cursorX, buffer.cursorY]", terminal));
		return new Position(pos[0], pos[1]);
	}
	
	final void write(TestBenchElement terminal, String text) {
		getCommandExecutor().executeScript(String.format("arguments[0].terminal.write('%s')",text), terminal);
	}
